...
NumericMatrix extract_all_binary_combinations(int n) {
	...
	NumericMatrix combo_mat(nTemp, n);

	// Loop through all possible values the CRF can take on 
	for (int i = 0; i < nTemp; i++) {
		// Loop through dimensions
		for (int k = 0; k < n; k++) {
			// The `(i, k)`th element is 1 iff the `k`th bit of `i` is 1, so `combo_mat`
			// lists all 2^`n` possible binary values that Z can take on in its
			// rows
			combo_mat(i, k) = (i >> k) & 1;
		}
	}
...
NumericMatrix extract_all_binary_combinations_ignoring_one_row(int n, int column_to_ignore) {
	...
	for (int row_num = 0; row_num < N; row_num++) {
		...
		for (int column_num = 0; column_num < n; column_num++) {
			...
			// If the column is not a column to ignore (i.e. a dimension we are marginalizing out)
			} else {
...
// Each row of this matrix summarizes all possible binary values that Z can take on assuming the values of column_to_ignore1 and column_to_ignore2 are fixed to be 1
NumericMatrix extract_all_binary_combinations_ignoring_two_row(int n, int column_to_ignore1, int column_to_ignore2) {
	// Special case (handeled separately) for when there are only two dimensions
	if (n == 2) {
		// Return 1x2 matrix of 1s
		NumericVector v = {1, 1};
		NumericMatrix combo_mat(1, 2, v.begin());
		return combo_mat;
	}
	// General case for when the number of dimensions is greater than 2
	// Ignoring column_to_ignore1 and column_to_ignore2, get matrix of dimension 
	// 2^(number_of_dimensions-2) x (number_of_dimensions-2) that representing 
	// all possible values CRF can take on for remaining dimensions
...
double exact_normalization_constant(NumericMatrix feat, StringMatrix discrete_outliers, NumericVector theta_singleton, NumericMatrix theta_pair, NumericMatrix theta, NumericMatrix phi_inlier, NumericMatrix phi_outlier, int number_of_dimensions, int sample_num, bool posterior_bool) {
	...
	for (int combination_number = 0; combination_number < all_binary_combinations_matrix.nrow(); combination_number++) {
		...
		double un_normalized_weight = un_normalized_crf_weight(all_binary_combinations_matrix, combination_number, feat, discrete_outliers, theta_singleton, theta_pair, theta, phi_inlier, phi_outlier, number_of_dimensions, sample_num, posterior_bool);
...
// Involves marginalizing out all other dimensions
double exact_marginal_pairwise_probability(double normalization_constant, int dimension1, int dimension2, int dimension_counter, NumericMatrix feat, StringMatrix discrete_outliers, NumericVector theta_singleton, NumericMatrix theta_pair, NumericMatrix theta, NumericMatrix phi_inlier, NumericMatrix phi_outlier, int number_of_dimensions, int sample_num, bool posterior_bool) {
...
// [[Rcpp::export]]
List update_marginal_probabilities_exact_inference_cpp(NumericMatrix feat, StringMatrix discrete_outliers, NumericVector theta_singleton, NumericMatrix theta_pair, NumericMatrix theta, NumericMatrix phi_inlier, NumericMatrix phi_outlier, int number_of_dimensions, int number_of_pairs, bool posterior_bool) {
	...
	for (int sample_num = 0; sample_num < feat.nrow(); sample_num++) {
		...
		for (int dimension = 0; dimension < number_of_dimensions; dimension++) {
			...
			for (int dimension2=dimension+1; dimension2 < number_of_dimensions; dimension2++) {
				...
				log_likelihood += theta_pair(0, dimension_counter) * 
								  posterior_pairwise(sample_num, dimension_counter);
				dimension_counter += 1;
			}
		}
	}
