...
// TODO: Delete all of these `_independent_` functions, which are redundant with
// ones in `crf_exact_updates.cpp`, but assume that `number_of_dimensions` is 1.
// However, the substitution should be trivial, as they already accept parameters
// associated with multiple dimensions, e.g. `theta_pair`.
